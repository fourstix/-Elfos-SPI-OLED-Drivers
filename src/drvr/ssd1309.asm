...
;
; Based on code from OLED-Display Library
...
; 
; Returns: rf - pointer to oled type string 'SSD1309'
;-------------------------------------------------------------------------------
...
dma_init:       db      $19 | $80     ; set low 6-bits of count
            ...
            ;---------------------------------------------
            ; Init sequence for SSD1309 displays.          
            ; Source: OLED-Display Library
...
init_start:     db      SET_DISP_OFF                    ; 0xAE,
                ...
                db      SET_PRECHARGE, $22              ; 0xD9, 0x22,
                db      SET_VCOM_DETECT, $20            ; 0xDB, 0x20,
...
OledType:       db 'SSD1309',0        ; oled type string for memory block id
